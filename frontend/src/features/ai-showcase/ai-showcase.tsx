            ...
            <div className="showcase-grid">
              <div>
                ...
                <Stack className="showcase-transcript" gap="lg" aria-live="polite">
                  ...
                  {latest?.done ? <Paper className="showcase-verdict" p="xl" radius="lg" role="status"><Text className="eyebrow">BJARNES ENDELIGE VURDERING</Text><Title order={3} mt="sm">{latest.status === "bjarne_lost" ? "Bjarne må innrømme nederlag" : latest.status === "possible_rejection" ? "Bjarne fant et mulig problem" : "Flere fakta trengs"}</Title>{latest.possibleIssue ? <Text mt="md">{latest.possibleIssue}</Text> : null}<Text mt="sm">{latest.reasoningSummary}</Text><Text c="dimmed" size="sm" mt="md">En leken vurdering, ikke en dekningsavgjørelse. Faktisk dekning avhenger av avtalen og vilkårene.</Text></Paper> : null}
                  <div ref={bottomRef} />
                </Stack>
