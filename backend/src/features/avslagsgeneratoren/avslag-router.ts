...
import type { AnswerRecord, InterrogationQuestion } from "./avslag-types.js";
...
avslagRouter.post("/sessions", async (request, response, next) => {
  ...
  try {
    ...
    const question = await createQuestion(trimmedClaim, []);
    const sessionId = createSessionId();
    sessions.set(sessionId, { claim: trimmedClaim, history: [], question, completed: false });
  ...
  try {
    if (session.history.length >= maxQuestions) {
      ...
      response.json({
        ...
        rejection: await createRejection(session.claim, session.history),
      });
      return;
    }

    session.question = await createQuestion(session.claim, session.history);
