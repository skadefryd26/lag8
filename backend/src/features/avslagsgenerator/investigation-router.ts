...
investigationRouter.post("/investigate", async (request, response, next) => {
  const { claim, turns, policyId } = request.body ?? {};
  ...
  if (!isPolicyId(policyId) || !validText(claim, 1500) || !Array.isArray(turns) || turns.length > maxAnswers ||
    ...
    response.status(400).json({ error: "Velg Reise eller Innbo-variant, skriv en kort fiktiv skademelding og svar på ett spørsmål om gangen (maks seks svar)." });
    return;
  }

  try {
    response.json(await investigate(claim.trim(), turns, policyId));
  } catch (error) {
    next(error);
  }
});
